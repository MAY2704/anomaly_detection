...
TensorFlow so the cross-platform matrix stays quick — please keep `input/`,
`output/evaluate.py`, and `output/alerts.py` free of TensorFlow imports. Only
`process/model.py`, `process/train.py`, and `output/inference.py` may need it.

`tests/test_import_isolation.py` enforces this by importing each module in a
subprocess with `keras` and `tensorflow` blocked. It has caught a real bug: a
convenience re-export in a package `__init__` pulled Keras into
`process.preprocess`, which passed everywhere TensorFlow happened to be
installed. If you add a public export backed by TensorFlow, export it lazily
(see `process/__init__.py`) rather than importing it at module scope.
...
- **Split by entity, never by window.** Sliding windows overlap, so
